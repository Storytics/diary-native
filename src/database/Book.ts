...
export const getAllActivity = async (): Promise<ActivityProps[]> =>
  new Promise((resolve, reject) => {
    Connection.transaction(
      (tx: SQLite.SQLTransaction) => {
        tx.executeSql(
          "SELECT book.title, page.createdAt, page.content, page.bookId, page.id FROM book INNER JOIN page ON page.bookId = book.id ORDER BY page.createdAt ASC LIMIT 25;",
          [],
          // @ts-ignore
          (_, { rows: { _array } }) => {
            resolve(_array);
